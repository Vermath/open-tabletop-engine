...
PostgreSQL, Redis, and MinIO bind to `127.0.0.1` by default so their development credentials are not exposed to the LAN. Keep `INFRA_BIND_ADDRESS=127.0.0.1` unless those ports are protected by a separate host firewall and authentication boundary; containers in the Compose network can reach the services without changing this host bind.
