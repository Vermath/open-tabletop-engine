...
| Deeper API behavior compatibility tests | API coverage now verifies expanded missing-session behavior across auth-management, organization/workspace, campaign/object, scene-tooling, chat/dice, combat, proposal, import/export, AI, plugin, system, and server-admin route families, plus read-only observer permission denials across common mutation/AI/plugin/system routes | Partially covered; still incomplete unless every shipped route gets behavior compatibility coverage or the remainder is explicitly descoped by owner |
...
- Deeper API behavior compatibility tests still need route-by-route coverage beyond served-contract, API-client conformance, and the expanded auth/permission matrix, or explicit owner descope.
