...
| Broader cross-organization administration/isolation | API coverage now verifies direct campaign/scenes reads, campaign archive, journal reads, asset metadata/blob access, actor/item updates, scene annotations, AI memory reads, unscoped chat listing, organization member rosters/member ids, and organization invite creation respect the active organization; campaign-invite acceptance creates organization membership plus an active organization session | Partially covered; still incomplete unless broader administration/isolation workflows are implemented or explicitly descoped by owner |
| Broader system-specific combat/rules automation | `docs/prd-v1-gap-closure.md` now records generic combat-state automation, timed combat condition expiry, Fighter healing, and metadata-backed class-feature condition outcomes such as D&D Monk Stunning Strike applying `Stunned`, but still calls out broader damage/healing outcomes, class-feature semantics/reactions, and broader rules-specific edge cases | Partially covered; still incomplete unless implemented or explicitly descoped by owner |
| Deeper API behavior compatibility tests | API coverage now compares the missing-session matrix to the served OpenAPI path/method surface, verifies every user-session-protected REST route is covered or explicitly excluded as public/browser-flow/SCIM-bearer, verifies read-only observer `403` outcomes across every non-self-service mutating user-session route from that served surface, verifies read-only observer outcomes across protected `GET` routes with explicit self-service auth and blob-fixture exclusions, verifies a privileged GM/server-admin read companion for observer-forbidden `GET` routes including admin operation reads and high-permission campaign reads, verifies a privileged server-admin mutating companion for admin `POST`/`PATCH`/`DELETE` routes with expected success or post-auth business statuses, and now verifies a privileged GM-positive companion across the non-admin observer-denied mutating fixture surface including auth-session revoke, organization invites/members, campaign lifecycle, fog/scene/token/actor/item/journal/chat/dice/combat/proposal/content-import/AI/plugin/system actions, plugin storage, plugin install/command, registry sync, and system actor action routes | Partially covered; missing-session route-surface, observer read/mutating outcome coverage, privileged read companion coverage, privileged admin mutation companion coverage, and privileged non-admin mutating permission-outcome coverage are complete for the current served user-session fixture surface, but deeper API behavior compatibility still needs broader payload/body permutations, generated-client behavior depth beyond current conformance, unmodeled route permutations, or explicit owner descope |
| Expanded advanced-flow browser E2E | `docs/prd-v1-gap-closure.md` now records browser coverage for timed combat condition expiry, but still calls out more advanced combat/rules interactions, broader import/export permutations, broader asset lifecycle/provider edge cases, niche plugin/system marketplace permutations, additional admin recovery actions, and deeper mobile interaction polish | Incomplete unless implemented or explicitly descoped by owner |
...
- Broader cross-organization administration/isolation needs implementation beyond the current owner/admin membership, workspace switching, workspace creation, direct route-scope enforcement, direct-object isolation, member/invite active-workspace scoping, and invite lifecycle slice, or explicit owner descope.
- Broader system-specific combat/rules automation needs implementation beyond the first generic combat-state, timed combat condition expiry, Fighter healing, and metadata-backed class-feature condition layer, or explicit owner descope.
- Deeper API behavior compatibility tests still need broader behavioral depth beyond served-contract, API-client conformance, OpenAPI-checked missing-session coverage, observer read/mutating outcome coverage, privileged read companion coverage, privileged admin mutation companion coverage, and privileged non-admin mutating permission-outcome coverage, such as payload/body permutations, unmodeled route permutations, or explicit owner descope.
- Expanded advanced-flow browser E2E needs implementation beyond the timed combat condition expiry slice for the remaining combat/rules, import/export, asset lifecycle/provider, plugin/system marketplace, admin recovery, and mobile interaction permutations, or explicit owner descope.
