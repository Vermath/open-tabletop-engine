...
| Broader cross-organization administration/isolation | API coverage now verifies direct campaign/scenes reads are denied when the target campaign belongs to a different active organization, and campaign-invite acceptance creates organization membership plus an active organization session | Partially covered; still incomplete unless broader administration/isolation workflows are implemented or explicitly descoped by owner |
| Broader system-specific combat/rules automation | `docs/prd-v1-gap-closure.md` still calls out broader condition expiry hooks, damage/healing outcomes, class-feature semantics, class-feature reactions, and broader rules-specific edge cases | Incomplete unless implemented or explicitly descoped by owner |
| Deeper API behavior compatibility tests | `docs/prd-v1-gap-closure.md` still calls out deeper behavior compatibility tests for every shipped route beyond served-contract and focused regression coverage | Incomplete unless implemented or explicitly descoped by owner |
...
- Broader cross-organization administration/isolation needs implementation beyond the current owner/admin membership, workspace switching, workspace creation, direct route-scope enforcement, and invite lifecycle slice, or explicit owner descope.
- Broader system-specific combat/rules automation needs implementation beyond the first generic combat-state and Fighter feature layer, or explicit owner descope.
- Deeper API behavior compatibility tests for every shipped route need implementation beyond served-contract and focused regression coverage, or explicit owner descope.
