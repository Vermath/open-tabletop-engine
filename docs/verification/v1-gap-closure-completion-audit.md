...
| Deeper API behavior compatibility tests | API coverage now compares the missing-session matrix to the served OpenAPI path/method surface, verifying every user-session-protected REST route is covered or explicitly excluded as public/browser-flow/SCIM-bearer; the same matrix verifies read-only observer `403` outcomes across every non-self-service mutating user-session route from that served surface, and now verifies read-only observer outcomes across protected `GET` routes with explicit self-service auth and blob-fixture exclusions | Partially covered; missing-session route-surface plus observer read/mutating outcome coverage are complete for the served user-session route surface, but broader route-by-route positive and negative permission-outcome matrices still need coverage or explicit owner descope |
...
- Deeper API behavior compatibility tests still need broader route-by-route positive and negative permission-outcome coverage beyond served-contract, API-client conformance, OpenAPI-checked missing-session coverage, and observer read/mutating outcome coverage, or explicit owner descope.
