...
| Deeper API behavior compatibility tests | API coverage now compares the missing-session matrix to the served OpenAPI path/method surface, verifying every user-session-protected REST route is covered or explicitly excluded as public/browser-flow/SCIM-bearer; read-only observer permission denials still cover common mutation/AI/plugin/system routes | Partially covered; missing-session route-surface coverage is complete for user-session routes, but broader route-by-route permission-outcome matrices still need coverage or explicit owner descope |
...
- Deeper API behavior compatibility tests still need broader route-by-route permission-outcome coverage beyond served-contract, API-client conformance, and OpenAPI-checked missing-session coverage, or explicit owner descope.
