...
| Deeper API behavior compatibility tests | API coverage now compares the missing-session matrix to the served OpenAPI path/method surface, verifying every user-session-protected REST route is covered or explicitly excluded as public/browser-flow/SCIM-bearer; the same matrix now verifies read-only observer `403` outcomes across every non-self-service mutating user-session route from that served surface, with explicit limited-session exceptions for authenticated creation/switch/import and read-permitted state changes | Partially covered; missing-session route-surface and mutating observer-denial coverage are complete for the served user-session route surface, but broader route-by-route positive and negative permission-outcome matrices still need coverage or explicit owner descope |
...
- Deeper API behavior compatibility tests still need broader route-by-route positive and negative permission-outcome coverage beyond served-contract, API-client conformance, OpenAPI-checked missing-session coverage, and mutating observer-denial coverage, or explicit owner descope.
