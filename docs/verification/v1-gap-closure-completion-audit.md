...
- Deeper API behavior compatibility tests still need broader route-by-route mutating positive permission-outcome coverage beyond served-contract, API-client conformance, OpenAPI-checked missing-session coverage, observer read/mutating outcome coverage, privileged read companion coverage, privileged admin mutation companion coverage, and the first GM-positive permission slice, or explicit owner descope.
