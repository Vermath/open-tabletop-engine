...
The repository's governing product thesis is an open-source, API-first, self-hostable virtual tabletop operating system. Campaign data must be portable; rules systems and plugins must be modular; secrets must be permissioned; and AI or plugin code must propose state changes for human approval instead of mutating campaign state directly.
...
| AI visibility and mutation safety | Player-facing provider context could include hidden prep, and dice/chat tools were modeled as direct side effects. | Provider context now excludes inactive prep scenes, unrevealed actors, prepared encounters, and unapproved memory for player seats. Dice and chat tools require `ai.proposeChanges` and return pending proposals; all campaign-edit tools still require their underlying domain permission. |
...
| Plugin product surface | Plugins had commands and storage, but no bounded event-subscription surface or safe asynchronous bridge. | Added validated manifest subscriptions, metadata-only redacted event envelopes, permission mapping, bounded queues and payloads, timeout/resource limits, and proposal/chat bridge receipts. Commands, event handlers, chat requests, and storage requests create pending proposals only. Trust, review, compatibility, grant, and campaign-scope checks fail closed. |
...
- AI and plugin code cannot directly mutate campaign state.
