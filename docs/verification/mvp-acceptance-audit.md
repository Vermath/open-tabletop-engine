...
| AI provider config, OpenAI/Codex adapter behavior, tool coverage, retries, status observability, encounter designer, recap, memory extraction, approval queue, and proposal application | `e37b252`, `df807e8`, `d86ed8b`, `5eb9aae`, the OpenAI Responses adapter slice, the AI tool depth and observability slice, and the AI integration reliability slice in `docs/verification/mvp-progress.md`; tests and manual evidence cover provider selection, permission-filtered context, typed OpenAI Responses tool schemas, Codex loopback proposal/encounter/memory/dice/compendium tools, provider retry/failure handling, thread status and duration metadata, tool-call observability, player mutation-tool denial, encounter/recap proposal approval and application, queued memory approval, and provider-backed memory extraction. |
...
- Codex integration hardening such as broader campaign-edit tools, deeper permission-regression breadth, cost/usage metrics, and richer operator dashboards; the MVP verifies provider abstraction, OpenAI Responses request/function-call mapping with typed tools, Codex loopback behavior, proposal/encounter/memory/dice/compendium tools, provider retry/failure handling, thread status history, tool-call observability, proposal application, and memory extraction.
