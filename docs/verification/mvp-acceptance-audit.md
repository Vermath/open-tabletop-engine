...
| AI provider config, OpenAI/Codex adapter behavior, tool coverage, retries, status observability, usage metrics, GM operator telemetry, encounter designer, recap, memory extraction, approval queue, and proposal application | `e37b252`, `df807e8`, `d86ed8b`, `5eb9aae`, the OpenAI Responses adapter slice, the AI tool depth and observability slice, the AI integration reliability slice, the AI usage metrics slice, and the AI operator dashboard slice in `docs/verification/mvp-progress.md`; tests and manual evidence cover provider selection, permission-filtered context, typed OpenAI Responses tool schemas, OpenAI provider usage mapping, Codex loopback proposal/encounter/memory/dice/compendium tools, provider retry/failure handling, thread status and duration metadata, token usage and estimated-cost aggregation, GM-only front-end operator telemetry, tool-call observability, player mutation-tool denial, encounter/recap proposal approval and application, queued memory approval, and provider-backed memory extraction. |
...
- Codex integration hardening such as broader campaign-edit tools and deeper permission-regression breadth; the MVP verifies provider abstraction, OpenAI Responses request/function-call mapping with typed tools and usage metrics, Codex loopback behavior, proposal/encounter/memory/dice/compendium tools, provider retry/failure handling, thread status history, estimated-cost aggregation, GM-only front-end operator telemetry, tool-call observability, proposal application, and memory extraction.
