...
| System runtime behavior with permission boundaries | `3a3e81a feat: add plugin and system runtimes` plus the rules compendium/conditions, character builder/advancement, encounter math, character importer, action automation, rest/resource automation, action resource consumption, target effect application, and D&D 5.5e SRD focus slices in `docs/verification/mvp-progress.md`; tests and browser/manual evidence now prioritize `dnd-5e-srd` with SRD 5.2.1 registration, Fighter/Cleric/Wizard templates, SRD compendium entries, character import normalization, level advancement, SRD threat planning, spell-slot-consuming actions, target HP healing/damage application, hit dice, spell slots, short/long rest recovery, and permission denial before resource mutation. Generic Fantasy, Stellar Frontiers, and Mystic Noir remain verified demo runtimes for later expansion, but the primary gap-closing rules direction is D&D SRD. |
