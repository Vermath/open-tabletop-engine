...
| Auth/session handling, password accounts, campaign invites, OIDC SSO, password reset/email delivery, first-class reset UI, TOTP MFA, SCIM provisioning, admin account/session operations, server-admin audit export, and REST/realtime/blob permissions | `3ce9a8d feat: add durable bearer sessions` plus the password accounts, campaign invites, OIDC SSO, production auth operations, production auth audit export, password reset UI, production auth MFA, and SCIM organization sync slices in `docs/verification/mvp-progress.md`; browser/API bearer evidence; API tests for login, logout, restart-safe token lookup, no-token `401`, password registration/login/change/reset, TOTP MFA enrollment/confirmation/login/recovery/disable, invite create/list/accept/revoke, OIDC config/start/callback with PKCE/state/user linking, SCIM bearer enforcement/user provisioning/deprovisioning/group sync, server-admin user/session/outbox controls, audit export filters/NDJSON/redaction, disabled legacy-header auth outside tests, session-token blob/realtime access, and Playwright reset-screen evidence. |
...
| AI provider config, OpenAI/Codex adapter behavior, tool coverage, observability, encounter designer, recap, memory extraction, approval queue, and proposal application | `e37b252`, `df807e8`, `d86ed8b`, `5eb9aae`, the OpenAI Responses adapter slice, and the AI tool depth and observability slice in `docs/verification/mvp-progress.md`; tests and manual evidence cover provider selection, permission-filtered context, typed OpenAI Responses tool schemas, Codex loopback proposal/encounter/memory/dice/compendium tools, tool-call observability, player mutation-tool denial, encounter/recap proposal approval and application, queued memory approval, and provider-backed memory extraction. |
...
- Enterprise IdP certification, group-to-campaign role mapping, and organization-admin UI beyond the verified SCIM user/group provisioning, API-backed account/session operations, first-class password reset screen, local TOTP MFA, and server-admin audit export.
...
- Codex integration hardening such as broader campaign-edit tools, permission-regression coverage, failure and retry handling, cost/latency dashboards, and operational observability; the MVP verifies provider abstraction, OpenAI Responses request/function-call mapping with typed tools, Codex loopback behavior, proposal/encounter/memory/dice/compendium tools, tool-call observability, proposal application, and memory extraction.
