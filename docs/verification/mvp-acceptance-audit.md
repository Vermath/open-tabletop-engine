...
- A complete rules engine for every game system; the MVP now prioritizes D&D 5.5e SRD with `dnd-5e-srd` registration, SRD 5.2.1-oriented templates, SRD background/species origin catalogs, create-time origin application, compendium entries, bounded expanded spell/weapon/armor entries, computed Armor Class from SRD armor/shield items, character import, guided advancement, action formulas, stateful spell-slot consumption, Healing Word/Cure Wounds/Chromatic Orb/Ray of Sickness upcasting, Ice Knife secondary damage upcasting, fixed and secondary spell damage quick rolls, saving throw quick rolls, class save proficiencies, skill and tool check quick rolls, class/template/background skill and tool proficiencies, SRD currency/equipment purchasing, Shortbow/Dagger/Quarterstaff/Spear weapon data, target damage/healing application, hit dice, spell slots, rest recovery, SRD monster stat blocks, monster actor creation, monster attack/damage quick rolls, CR/XP encounter threat budgets, and 2024 Basic Rules encounter XP thresholds. Remaining non-MVP D&D work is full SRD-scale spell/equipment/magic-item content coverage, deeper class/subclass feature automation, more complete species trait automation, and full SRD-scale monster/action coverage beyond the current SRD threat catalog. Generic Fantasy, Stellar Frontiers, and Mystic Noir remain demo runtimes for later expansion.
