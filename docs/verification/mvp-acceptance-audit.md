...
| Auth/session handling, password accounts, campaign invites, OIDC SSO, password reset/email delivery, admin account/session operations, and REST/realtime/blob permissions | `3ce9a8d feat: add durable bearer sessions` plus the password accounts, campaign invites, OIDC SSO, and production auth operations slices in `docs/verification/mvp-progress.md`; browser/API bearer evidence; API tests for login, logout, restart-safe token lookup, no-token `401`, password registration/login/change/reset, invite create/list/accept/revoke, OIDC config/start/callback with PKCE/state/user linking, server-admin user/session/outbox controls, disabled legacy-header auth outside tests, and session-token blob/realtime access. |
| Campaigns, scenes, maps/assets, tokens, dice, chat, actors, sheets, journals, combat, fog/walls/lights | Verified milestone evidence for uploaded maps, signed asset delivery, quotas, lifecycle state, storage stats, GM/player session switching, hidden-token filtering, fog/vision filtering, token movement ownership, fog/wall/light authoring, dice/chat/API tests, actor sheet UI, journal visibility tests, combat UI/API, and clean-checkout role/ownership acceptance in `docs/verification/mvp-progress.md`. |
...
| AI provider config, OpenAI/Codex adapter behavior, encounter designer, recap, memory extraction, approval queue, and proposal application | `e37b252`, `df807e8`, `d86ed8b`, `5eb9aae`, and the OpenAI Responses adapter slice in `docs/verification/mvp-progress.md`; tests and manual evidence cover provider selection, permission-filtered context, Codex loopback proposal tool, OpenAI Responses request/function-call mapping, encounter/recap proposal approval and application, queued memory approval, and provider-backed memory extraction. |
...
- Hosted-model prompt quality and broad hosted-tool evals; the MVP verifies provider abstraction, OpenAI Responses request/function-call mapping, Codex loopback behavior, tools, proposal application, and memory extraction.
