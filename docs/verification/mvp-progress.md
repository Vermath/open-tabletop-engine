...
### 2026-05-01 - D&D SRD Monk automation slice

- Implementation:
  - Added a first-class D&D 5.5e SRD Monk character template with Dexterity/Wisdom-oriented attributes, D8 hit die, Strength/Dexterity saves, Acrobatics/Stealth skills, Musical Instrument tool proficiency, Spear/Dagger/Musical Instrument starter equipment, and SRD 5.2.1 attribution metadata.
  - Added the SRD Musical Instrument compendium tool entry and Monk class feature progression through level 5: Martial Arts, Unarmored Defense, Monk's Focus, Flurry of Blows, Patient Defense, Step of the Wind, Unarmored Movement, Uncanny Metabolism, Deflect Attacks, Monk Subclass, Ability Score Improvement, Slow Fall, Extra Attack, and Stunning Strike.
  - Added Monk Martial Arts unarmed and weapon damage formulas, Unarmored Defense AC calculation, Focus and Uncanny Metabolism resources, Unarmored Movement metadata, Extra Attack metadata, and class-feature quick rolls for Martial Arts Damage, Flurry of Blows, Patient Defense, Step of the Wind, Uncanny Metabolism Healing, Deflect Attacks Damage, and Stunning Strike.
  - Added stateful Focus/Uncanny Metabolism consumption and recovery through the D&D action/rest paths, target damage application for Deflect Attacks, self-healing for Uncanny Metabolism, Monk save DC metadata, API roll/resource consumption, and browser action-list entries.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test -- --run` passed with `24 passed`.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test -- src/app.test.ts -t "D&D 5.5e SRD"` passed with `1 passed | 60 skipped` after rebuilding the SDK package used by the API.
  ...
  - `git diff --check` passed with only the expected LF-to-CRLF working-copy warnings.
  - `pnpm check` passed across lint, typecheck, tests, and build.
  - System SDK tests verify Monk template data, saves/skills/tools/items, Musical Instrument compendium data, Unarmored Defense AC, Martial Arts unarmed and weapon formulas, level-5 progression, Focus and Uncanny Metabolism resources, Extra Attack metadata, Focus-consuming actions, depletion protection, Short Rest Focus recovery, and Long Rest Uncanny Metabolism restoration.
  - API tests verify the Monk template appears in D&D SRD character templates, level-1 and level-5 Monk sheets expose Martial Arts/Focus/Uncanny/Deflect/Stunning metadata, Flurry and Deflect consume Focus, Deflect can apply target HP damage, depleted Stunning Strike returns `409`, Short Rest restores Focus, Uncanny Metabolism restores Focus while consuming its Long Rest resource, and Long Rest restores Uncanny Metabolism.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and real bearer tokens from `POST /auth/login` for GM and player sessions; no legacy `x-user-id` fallback was used.
  - The GM bearer created a player-owned `Manual Monk Automation` from the Monk template, returning `200`, actor `act_montk0xhyd2odi3z`, features `["Martial Arts", "Unarmored Defense"]`, resources `{}`, Armor Class `15`, inventory `Spear`, `Dagger`, and `Musical Instrument`, Martial Arts formula `1d6+3`, and Spear damage formula `1d6+3` with Martial Arts metadata.
  - Four player-bearer `POST /advance` calls advanced the Monk to level `5` with features `["Martial Arts", "Unarmored Defense", "Monk Level 2", "Monk's Focus", "Flurry of Blows", "Patient Defense", "Step of the Wind", "Unarmored Movement", "Uncanny Metabolism", "Monk Level 3", "Deflect Attacks", "Monk Subclass", "Monk Level 4", "Ability Score Improvement", "Slow Fall", "Monk Level 5", "Extra Attack", "Stunning Strike"]`.
  - The level-5 actor had resources `{ focus: { current: 2, max: 5, recovery: "short" }, uncannyMetabolism: { current: 1, max: 1, recovery: "long" } }` and combat metadata `{ attacksPerAction: 2, unarmoredMovement: { bonusFt: 10, armorRestriction: "not wearing armor or wielding a Shield" } }`.
  - The level-5 sheet exposed Martial Arts `1d8+5`, Uncanny Metabolism `1d8+5`, Deflect Attacks `2d8+5` with reduction formula `1d10+5+5`, and Stunning Strike with Constitution save DC `13`.
  - A player-bearer Flurry of Blows roll with `consumeResources: true` returned `200` and consumed Focus to remaining `1`.
  - A player-bearer Deflect Attacks roll with `consumeResources: true`, `applyEffect: true`, and target `act_montk0xyz04iummm` returned `200`, formula `2d8+5`, consumed Focus to remaining `0`, and applied HP damage to the target.
  - A follow-up Stunning Strike roll with depleted Focus returned `409` with `Insufficient focus point`.
  - A player-bearer Short Rest returned `200`, recovered Focus to `5`, and left Uncanny Metabolism available.
  - A player-bearer Uncanny Metabolism roll returned `200`, formula `1d8+5`, consumed Uncanny Metabolism to remaining `0`, restored Focus to `5`, and applied self-healing.
  - A player-bearer Long Rest returned `200`, restored resources to `{ focus: { current: 5, max: 5, recovery: "short" }, uncannyMetabolism: { current: 1, max: 1, recovery: "long" } }`.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD Sorcerer automation slice

- Implementation:
  - Added a first-class D&D 5.5e SRD Sorcerer character template with Charisma-oriented attributes, d6 hit die, Constitution/Charisma saves, Arcana/Persuasion skills, Calligrapher's Supplies proficiency, Sorcerous Burst/Chromatic Orb/Shield spells, Spear/Dagger/Arcane Focus starter equipment, level-1 spell slots, and Innate Sorcery resources.
  - Added SRD compendium entries for Sorcerous Burst and Arcane Focus.
  - Added Sorcerer class feature progression through level 5: Spellcasting, Innate Sorcery, Font of Magic, Metamagic, Sorcerer Subclass, Ability Score Improvement, and Sorcerous Restoration.
  - Added class-feature quick rolls for `feature-innate-sorcery`, `feature-convert-spell-slot-to-sorcery-points`, `feature-create-spell-slot`, `feature-metamagic-empowered-spell`, and `feature-metamagic-quickened-spell`.
  - Added stateful Innate Sorcery, Sorcery Point, Flexible Casting, Metamagic, and Sorcerous Restoration consumption/recovery through the D&D action/rest paths, plus browser action-list entries.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test -- --run` passed with `24 passed`.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test -- src/app.test.ts -t "D&D 5.5e SRD"` passed with `1 passed | 60 skipped` after rebuilding the SDK package used by the API.
  ...
  - `git diff --check` passed with only the expected LF-to-CRLF working-copy warnings.
  - `pnpm check` passed across lint, typecheck, tests, and build.
  - System SDK tests verify Sorcerer template data, saves/skills/tool/items, Sorcerous Burst and Arcane Focus compendium data, Charisma spellcasting formulas, level-5 progression, Innate Sorcery/Sorcery Points/Sorcerous Restoration resources, Flexible Casting metadata, Metamagic metadata, stateful resource consumption, depletion protection, Short Rest Sorcerous Restoration, and Long Rest recovery.
  - API tests verify the Sorcerer template appears in D&D SRD character templates, level-1 and level-5 Sorcerer sheets expose Innate Sorcery/Flexible Casting/Metamagic metadata, Innate Sorcery and Quickened Spell consume resources, Create Spell Slot and Convert Spell Slot mutate spell slots/Sorcery Points, depleted Quickened Spell returns `409`, Short Rest triggers Sorcerous Restoration, and Long Rest restores resources and spell slots.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and a real bearer token from `POST /auth/login` as `usr_demo_gm`; no legacy `x-user-id` fallback was used.
  - The GM bearer created `Manual SRD Sorcerer` from the Sorcerer template, returning `200`, actor `act_monu9gh0eqw7a9rg`, features `["Spellcasting", "Innate Sorcery"]`, resources `{ innateSorcery: { current: 2, max: 2, recovery: "long" } }`, level-1 spell slots `{ level1: { current: 2, max: 2, recovery: "long" } }`, and inventory `Sorcerous Burst`, `Chromatic Orb`, `Shield`, `Spear`, `Dagger`, and `Arcane Focus`.
  - A bearer quick-roll check on actor `act_monu9x8d21ocrgv6` confirmed `feature-innate-sorcery` with spell save DC `14`, `Sorcerous Burst Damage` formula `1d8`, and `Chromatic Orb Damage` formula `3d8`.
  - Four bearer `POST /advance` calls advanced actor `act_monu9gh6o06l1e3i` to level `5` with Font of Magic, Metamagic, Sorcerer Subclass, Ability Score Improvement, and Sorcerous Restoration; resources were `{ innateSorcery: { current: 2, max: 2, recovery: "long" }, sorceryPoints: { current: 2, max: 5, recovery: "long" }, sorcerousRestoration: { current: 1, max: 1, recovery: "long" } }`.
  - The level-5 sheet exposed Innate Sorcery with spell save DC `17`, Convert Spell Slot available levels `[1, 2, 3]`, Create Spell Slot costs through level-3 slots, Empowered Spell cost `1`, and Quickened Spell cost `2`.
  - On action actor `act_monu9ghgpwar9mx5`, a bearer Innate Sorcery roll with `consumeResources: true` returned `200`, formula `0`, and consumed Innate Sorcery to remaining `1`.
  - A bearer Quickened Spell roll returned `200` and consumed Sorcery Points from `5` to `3`; a depleted follow-up returned `409` with `Insufficient sorcery points`.
  - A bearer Create Spell Slot roll with `spellSlotLevel: 1` returned `200`, consumed `2` Sorcery Points, and restored level-1 spell slots from `2` to `3`.
  - A bearer Convert Spell Slot roll with `spellSlotLevel: 2` returned `200`, consumed a level-2 spell slot from `2` to `1`, and restored Sorcery Points from `3` to `5`.
  - A bearer Short Rest returned `200`, restored Sorcery Points from `0` to `2`, and consumed Sorcerous Restoration to `0`; a bearer Long Rest returned `200` and restored Innate Sorcery, Sorcery Points, Sorcerous Restoration, and level-1/2/3 spell slots to full.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD Warlock automation slice

- Implementation:
  - Added a first-class D&D 5.5e SRD Warlock character template with Charisma-oriented attributes, d8 hit die, Wisdom/Charisma saves, Arcana/Intimidation skills, Eldritch Blast/Hex spells, Leather Armor/Sickle/Dagger/Arcane Focus starter equipment, level-1 Pact Magic slots, and Eldritch Invocations.
  - Added SRD compendium entries for Eldritch Blast, Hex, and Sickle.
  - Added Warlock class feature progression through level 5: Eldritch Invocations, Pact Magic, Magical Cunning, Warlock Subclass, and Ability Score Improvement.
  - Added class-feature quick rolls for `feature-eldritch-invocations` and `feature-magical-cunning`.
  - Added stateful Pact Magic spell-slot normalization, default Pact Magic slot consumption for Warlock spells, Short Rest Pact Magic recovery, Magical Cunning half-slot recovery and depletion protection, Long Rest Magical Cunning recovery, and browser action-list entries.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test -- --run` passed with `25 passed`.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test -- src/app.test.ts -t "D&D 5.5e SRD"` passed with `1 passed | 60 skipped` after rebuilding the SDK package used by the API.
  - `pnpm --filter @open-tabletop/web build` passed.
  - `pnpm --filter @open-tabletop/api build` passed before manual API smoke.
  - System SDK tests verify Warlock template data, saves/skills/items, Eldritch Blast/Hex/Sickle compendium data, invocation metadata, level-5 Pact Magic slot replacement, Magical Cunning metadata, default level-3 Pact Magic consumption for Hex, Magical Cunning restoration/depletion, Short Rest Pact Magic recovery, and Long Rest Magical Cunning recovery.
  - API tests verify the Warlock template appears in D&D SRD character templates, level-1 and level-5 Warlock sheets expose Eldritch Invocations/Magical Cunning metadata, Hex consumes a level-3 Pact Magic slot at level 5, Magical Cunning restores one expended Pact Magic slot and consumes its resource, depleted Magical Cunning returns `409`, Short Rest restores Pact Magic slots, and Long Rest restores Magical Cunning and Pact Magic slots.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and a real bearer token from `POST /auth/login` as `usr_demo_gm`; no legacy `x-user-id` fallback was used.
  - The GM bearer created `Manual SRD Warlock` from the Warlock template, returning `200`, actor `act_monuy0xsmtjuav5k`, features `["Eldritch Invocations", "Pact Magic"]`, no level-1 resource pool, Pact Magic slots `{ level1: { current: 1, max: 1, recovery: "short" } }`, inventory `Leather Armor`, `Sickle`, `Dagger`, and `Arcane Focus`, and spells `Eldritch Blast` and `Hex`.
  - A bearer quick-roll check confirmed `feature-eldritch-invocations` with `known: 1` and Pact of the Blade/Chain/Tome metadata, `Eldritch Blast Damage` formula `1d10`, and `Hex Damage` formula `1d6`.
  - Four bearer `POST /advance` calls advanced actor `act_monuy0xyy4x5sdma` to level `5` with Eldritch Invocations, Pact Magic, Magical Cunning, Warlock Subclass, and Ability Score Improvement; resources were `{ magicalCunning: { current: 1, max: 1, recovery: "long" } }` and spell slots were `{ level3: { current: 2, max: 2, recovery: "short" } }`.
  - The level-5 sheet exposed `feature-eldritch-invocations` with `known: 5` and `feature-magical-cunning` with max recovered slots `1`, Pact Magic slot level `3`, max slots `2`, and Short Rest recovery metadata.
  - A bearer Hex roll with `consumeResources: true` and no explicit `spellSlotLevel` returned `200`, formula `1d6`, selected Pact Magic `slotLevel: 3`, and consumed `level3` slots from `2` to `1`.
  - A bearer Magical Cunning roll on depleted Pact Magic returned `200`, consumed Magical Cunning to `0`, and restored `level3` Pact Magic slots from `0` to `1`; a depleted follow-up returned `409` with `Insufficient magical cunning`.
  - A bearer Short Rest returned `200`, recovered Pact Magic slots by `{ level3: 2 }`, left Magical Cunning at `0`, and restored level-3 Pact Magic slots to full; a bearer Long Rest returned `200` and restored Magical Cunning plus level-3 Pact Magic slots to full.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD species trait automation slice

- Implementation:
  - Added first-class D&D SRD species trait quick rolls and stateful resources for Dragonborn Breath Weapon and Draconic Flight, Dwarf Stonecunning and Dwarven Toughness, Goliath Giant Ancestry and Large Form, and Orc Adrenaline Rush and Relentless Endurance.
  - Added actor-aware formulas and metadata for Breath Weapon scaling, save DC, shape, damage type choice, Adrenaline Rush temporary hit points, Stonecunning Tremorsense, Goliath Giant Ancestry options, Large Form speed, and Relentless Endurance trigger/result text.
  - Wired species resources into default D&D SRD character resources, create-time origin application, level advancement, action formula resolution, roll resource consumption, Short/Long Rest recovery, and browser D&D action options.
  - Added Dwarven Toughness HP application at character creation and Dwarf level-up advancement.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test -- --run` passed with `25 passed`.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test -- src/app.test.ts -t "D&D 5.5e SRD"` passed with `1 passed | 60 skipped`.
  ...
  - `git diff --check` passed with only the expected LF-to-CRLF working-copy warnings.
  - `pnpm check` passed across lint, typecheck, tests, and build.
  - System SDK tests verify Orc Adrenaline Rush/Relentless Endurance quick rolls and Short Rest recovery, Dwarf Stonecunning and Dwarven Toughness HP at creation/level-up, Dragonborn Breath Weapon/Draconic Flight at level 5 with Breath Weapon consumption, and Goliath Giant Ancestry/Large Form at level 5.
  - API tests verify player-owned Orc, Dwarf, and Dragonborn species trait quick rolls, resource consumption, Short Rest recovery, Dwarven Toughness HP, Dragonborn level-5 Breath Weapon scaling, and permissioned player rolls through the system runtime.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and real GM/player bearer tokens from `POST /api/v1/auth/login`; both logins returned `200` with `ots_` token prefixes, and no legacy `x-user-id` fallback was used.
  - The GM bearer created player-owned `Manual SRD Orc Species`, returning `200`, actor `act_monvj3tb1fo8asbr`, resources `{ adrenalineRush: { current: 2, max: 2, recovery: "short" }, relentlessEndurance: { current: 1, max: 1, recovery: "long" } }`, and quick rolls `species-orc-adrenaline-rush` formula `2` plus `species-orc-relentless-endurance` formula `0`.
  - A player-bearer Orc Adrenaline Rush roll with `consumeResources: true` returned `200` and consumed `{ key: "adrenalineRush", amount: 1, remaining: 1 }`; a player-bearer Short Rest returned `200`, recovered `{ adrenalineRush: 1 }`, and restored Adrenaline Rush to full.
...
- Implementation:
  - Added SRD compendium entries for Dissonant Whispers, Dragon's Breath, Mind Spike, Ensnaring Strike, and Starry Wisp, including level, school, action, range, damage formula, upcast formula where applicable, damage type, class metadata, concentration/duration metadata, save metadata, and rider metadata.
  - Wired the new spell entries through the existing compendium-backed spell quick-roll and spell-slot consumption path rather than adding a separate rules subsystem.
  - Extended D&D API coverage for compendium listing, add-to-actor behavior, quick-roll formulas, and Dissonant Whispers level-3 upcast slot consumption.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test -- --run` passed with `25 passed`.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test -- src/app.test.ts -t "D&D 5.5e SRD"` first confirmed the API test consumes built SDK output, then passed with `1 passed | 60 skipped` after rebuilding `@open-tabletop/system-sdk`.
  - `pnpm --filter @open-tabletop/api build` passed before manual API smoke.
  - `pnpm --filter @open-tabletop/web build` passed.
  - `pnpm --filter @open-tabletop/ai-core build` passed.
  - `git diff --check` passed with only the expected LF-to-CRLF working-copy warnings.
  - `pnpm check` passed across lint, typecheck, tests, and build; API tests reported `68 passed`, and system SDK tests reported `25 passed`.
  - System SDK tests verify new spell compendium data plus upcast formulas for Dissonant Whispers, Dragon's Breath, and Mind Spike.
  - API tests verify the new spells appear in the D&D SRD compendium, Dissonant Whispers can be added to an actor from the compendium, the sheet exposes its damage quick roll, and a level-3 Dissonant Whispers roll consumes a level-3 slot with formula `3d6+2d6`.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and real GM/player bearer tokens from `POST /api/v1/auth/login`; both logins returned `200` with `ots_` token prefixes, and no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/compendium` returned `200`, `40` entries, and included Dissonant Whispers `3d6` plus `1d6` upcast with Wisdom half save, Dragon's Breath `3d6` plus `1d6` upcast with Dexterity half save, Mind Spike `3d8` plus `1d8` upcast with Wisdom half save, Ensnaring Strike `1d6` plus `1d6` upcast with Strength save, and Starry Wisp `1d8` radiant.
  - The GM bearer created player-owned `Manual SRD Expanded Spells`, returning `200`, actor `act_monw3l7mpa7ujt3e`; four player-bearer advances reached level `5` with spell slots `{ level1: { current: 2, max: 4, recovery: "long" }, level2: { current: 2, max: 3, recovery: "long" }, level3: { current: 2, max: 2, recovery: "long" } }`.
  - The GM bearer added all five new spell entries to that actor, each returning `200`, with item ids `itm_monw3l80sc4uofa5`, `itm_monw3l82imde284n`, `itm_monw3l83c6iptgqu`, `itm_monw3l8421sdmpb0`, and `itm_monw3l86ehuau8sl`.
  - A player-bearer Dissonant Whispers roll with `spellSlotLevel: 3` and `consumeResources: true` returned `200`, formula `3d6+2d6`, selected `slotLevel: 3`, and consumed `level3` to remaining `1`.
  - A player-bearer Dragon's Breath roll with `spellSlotLevel: 3` and `consumeResources: true` returned `200`, formula `3d6+1d6`, selected `slotLevel: 3`, and consumed `level3` to remaining `0`.
  - A player-bearer Starry Wisp roll with `consumeResources: true` returned `200`, formula `1d8`, and consumed no spell slot.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD remaining added-spell compendium and full-caster slot slice

- Implementation:
  - Added SRD compendium entries for Aura of Life, Charm Monster, Elementalism, Phantasmal Force, Power Word Heal, Searing Smite, Summon Dragon, Tsunami, and Vitriolic Sphere, including metadata for saves, conditions, concentration, areas, healing, summons, recurring damage/healing, secondary damage, and upcasting where the SRD spell supports it.
  - Replaced the early-level-only Bard/Cleric/Druid/Sorcerer/Wizard spell-slot stub with the SRD full-caster slot table through level 20, so level-9 full casters now expose natural level-5 slots for supported high-slot spell actions.
  - Extended System SDK and D&D API tests for the new spell entries, Phantasmal Force/Searing Smite/Tsunami/Vitriolic Sphere formulas, and natural level-9 Wizard level-5 slot consumption.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test -- --run` passed with `25 passed`.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test -- src/app.test.ts -t "D&D 5.5e SRD"` passed with `1 passed | 60 skipped` after rebuilding `@open-tabletop/system-sdk`.
  - `pnpm --filter @open-tabletop/api build` passed before manual API smoke.
  - `pnpm --filter @open-tabletop/web build` passed.
  - `pnpm --filter @open-tabletop/ai-core build` passed.
  - `git diff --check` passed with only the expected LF-to-CRLF working-copy warnings.
  - `pnpm check` passed across lint, typecheck, tests, and build; API tests reported `68 passed`, and system SDK tests reported `25 passed`.
  - System SDK tests verify the nine spell compendium entries, formulas for Phantasmal Force, Searing Smite, Tsunami, and Vitriolic Sphere, SRD full-caster level-9 slots, and Vitriolic Sphere level-5 slot consumption without test-only slot injection.
  - API tests verify the nine spell entries appear in the D&D SRD compendium, Vitriolic Sphere add-to-actor behavior exposes initial and secondary damage quick rolls, and a level-5 Vitriolic Sphere roll consumes a level-5 slot.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and real GM/player bearer tokens from `POST /api/v1/auth/login`; both logins returned `200` with `ots_` token prefixes, and no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/compendium` returned `200`, `49` entries, and included Aura of Life, Charm Monster, Elementalism, Phantasmal Force, Power Word Heal, Searing Smite, Summon Dragon, Tsunami, and Vitriolic Sphere with the expected level/formula/condition/healing/summon metadata.
  - The GM bearer created player-owned `Manual SRD Remaining Added Spells`, returning `200`, actor `act_monwkl3wseite6zb`; eight player-bearer advances reached level `9` with full-caster spell slots `{ level1: { current: 2, max: 4 }, level2: { current: 2, max: 3 }, level3: { current: 2, max: 3 }, level4: { current: 1, max: 3 }, level5: { current: 1, max: 1 } }`.
  - The GM bearer added all nine new spell entries to that actor, each returning `200`, with item ids `itm_monwkl4eze3u07lr`, `itm_monwkl4ff78d0tfd`, `itm_monwkl4g1n03kg1i`, `itm_monwkl4hjc77ubfb`, `itm_monwkl4i6u3buykz`, `itm_monwkl4jtdq36sxj`, `itm_monwkl4lruolvpvz`, `itm_monwkl4m3844j0d6`, and `itm_monwkl4n7oog0cai`.
  - A player-bearer Phantasmal Force roll with `consumeResources: true` returned `200`, formula `2d8`, selected `slotLevel: 2`, and consumed `level2` to remaining `1`.
  - A player-bearer Searing Smite roll with `spellSlotLevel: 3` and `consumeResources: true` returned `200`, formula `1d6+2d6`, selected `slotLevel: 3`, and consumed `level3` to remaining `1`.
  - A player-bearer Vitriolic Sphere roll with `spellSlotLevel: 5` and `consumeResources: true` returned `200`, formula `10d4+2d4`, selected `slotLevel: 5`, and consumed the naturally advanced Wizard `level5` slot to remaining `0`.
  - Player-bearer secondary/non-consuming formula checks returned `200` for Tsunami initial `6d10`, Tsunami secondary `5d10`, and Vitriolic Sphere secondary `5d4`.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD core spell compendium and cantrip scaling slice

- Implementation:
  - Added SRD compendium entries for Acid Arrow, Acid Splash, Aid, Bane, Bless, Burning Hands, Color Spray, Command, Guiding Bolt, Magic Missile, and Thunderwave, including spell level, school, action, range, save/attack metadata, class metadata, area/target metadata, condition/command/rider metadata, and upcast metadata where the SRD spell supports it.
  - Added actor-level cantrip scaling metadata and formula resolution for Acid Splash, Fire Bolt, Sorcerous Burst, Eldritch Blast, and Starry Wisp at levels 5, 11, and 17.
  - Extended spell formula scaling for numeric formulas and dice-plus-constant formulas so Aid and Magic Missile scale through the same `spellSlotLevel` path as other D&D SRD spell actions.
  - Updated D&D Fire Bolt to resolve as a spell attack cantrip without a weapon-style ability modifier.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test` passed with `25` tests.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test` passed with `2` files and `68` tests after rebuilding `@open-tabletop/system-sdk`.
  - `pnpm --filter @open-tabletop/api build` passed before the manual in-memory API smoke.
  - `pnpm --filter @open-tabletop/web build` passed.
  - `pnpm --filter @open-tabletop/ai-core build` passed.
  - `git diff --check` passed with only the expected LF-to-CRLF working-copy warnings.
  - `pnpm check` passed across lint, typecheck, tests, and build; lint and typecheck each reported `22` successful tasks, tests reported `22` successful tasks with API `68` passed and system SDK `25` passed, and build reported `14` successful tasks.
  - System SDK tests verify the new compendium entries, Acid Arrow initial/secondary upcasts, Acid Splash cantrip scaling, Aid numeric upcasting, Burning Hands, Guiding Bolt, Magic Missile dice-plus-constant upcasting, and Thunderwave.
  - API tests verify the new entries appear in the D&D SRD compendium, Magic Missile can be added to an actor from the compendium, and a level-3 Magic Missile roll resolves `3d4+3+2d4+2`.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and real GM/player bearer tokens from `POST /api/v1/auth/login`; both logins returned `200`, and no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/compendium` returned `200`, `60` entries, and included Acid Arrow, Acid Splash, Aid, Bane, Bless, Burning Hands, Color Spray, Command, Guiding Bolt, Magic Missile, and Thunderwave with the expected level/formula/upcast metadata.
  - The GM bearer created player-owned `Manual SRD Core Spells`, returning `200`, actor `act_monx8led2juhyi37`; four player-bearer `POST /advance` calls reached level `5` with spell slots `{ level1: { current: 2, max: 4 }, level2: { current: 2, max: 3 }, level3: { current: 2, max: 2 } }`.
  - The GM bearer added Acid Splash, Acid Arrow, Aid, Burning Hands, Guiding Bolt, Magic Missile, and Thunderwave to that actor, each returning `200`, with item ids `itm_monx8leslsx6siqg`, `itm_monx8levdimja8kh`, `itm_monx8lex872wvnhk`, `itm_monx8leymij4de1h`, `itm_monx8lf0yc0hsw9g`, `itm_monx8lf2xyejjv1n`, and `itm_monx8lf3b1bctqi3`.
  - Player-bearer non-consuming roll checks returned Acid Splash level-5 formula `2d6`, Acid Arrow level-3 initial formula `4d4+1d4`, Acid Arrow level-3 secondary formula `2d4+1d4`, Aid level-4 formula `5+10`, Burning Hands level-2 formula `3d6+1d6`, Guiding Bolt level-3 formula `4d6+2d6`, and Thunderwave level-3 formula `2d8+2d8`.
  - A player-bearer Magic Missile roll with `spellSlotLevel: 3` and `consumeResources: true` returned `200`, formula `3d4+3+2d4+2`, selected `slotLevel: 3`, and consumed `level3` to remaining `1`.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD armor and weapon compendium slice

- Implementation:
  - Expanded the D&D SRD compendium to cover the mundane armor table with Padded Armor, Leather Armor, Studded Leather Armor, Hide Armor, Chain Shirt, Scale Mail, Breastplate, Half Plate Armor, Ring Mail, Chain Mail, Splint Armor, Plate Armor, and Shield.
  - Expanded the D&D SRD weapon table coverage across simple/martial and melee/ranged weapons, including Club, Dagger, Greatclub, Handaxe, Javelin, Light Hammer, Mace, Quarterstaff, Sickle, Spear, Dart, Light Crossbow, Shortbow, Sling, Battleaxe, Flail, Glaive, Greataxe, Greatsword, Halberd, Lance, Longsword, Maul, Morningstar, Pike, Rapier, Scimitar, Shortsword, Trident, Warhammer, War Pick, Whip, Blowgun, Hand Crossbow, Heavy Crossbow, Longbow, Musket, and Pistol.
  - Added SRD equipment metadata needed by the runtime: armor type/base AC/Dexterity caps/Strength requirements/stealth disadvantage/don-doff action, weapon category/kind/range/ammunition/properties/versatile damage/damage type, and weapon mastery tags.
  - Reused the existing purchase, sheet AC, and item quick-roll paths so purchased or compendium-added armor and weapons immediately affect actor sheets and action formulas.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test` passed with `25` tests.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  ...
  - `git diff --check` passed with only the expected LF-to-CRLF working-copy warnings.
  - `pnpm check` passed across lint, typecheck, tests, and build; lint and typecheck each reported `22` successful tasks, tests reported `22` successful tasks with API `68` passed and system SDK `25` passed, and build reported `14` successful tasks.
  - System SDK tests verify the armor and weapon entries, medium/heavy armor AC behavior, stealth/Strength metadata, Light Crossbow and Greatsword quick-roll formulas, Extra Attack metadata, Scale Mail purchasing, and Handaxe purchasing.
  - API tests verify representative armor and weapon entries appear in the D&D SRD compendium, Handaxe purchasing spends currency and exposes weapon metadata, and the purchased Handaxe quick roll resolves through the REST sheet path.
- Manual API evidence:
  ...
  - `GET /systems/dnd-5e-srd/compendium` returned `200`, `98` entries, and included all `38` newly checked armor/weapon ids from the slice.
  - GM-bearer compendium add checks put Light Crossbow, Greatsword, and Plate Armor on a Fighter, then roll checks returned Light Crossbow `1d8+1` and Greatsword `2d6+3`.
  - The same Fighter sheet returned Plate Armor AC `18` with `armorName: "Plate Armor"`, `base: 18`, `dexModifier: 0`, `stealthDisadvantage: true`, `strengthRequirement: 15`, and no speed penalty for Strength 16.
  - A fresh Fighter purchased Scale Mail for `50` GP, leaving `0` GP, with item data `armorBase: 14`, `dexCap: 2`, and `stealthDisadvantage: true`; the returned sheet AC was `15`.
  - A fresh Cleric purchased two Handaxes for `10` GP, leaving `40` GP, with item data `damage: "1d6"`, `range: "20/60"`, `mastery: "vex"`, and a returned Handaxe quick roll of `1d6+0`.
  - Source rule references for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf, and D&D Beyond 2024 Basic Rules equipment page, https://www.dndbeyond.com/sources/dnd/br-2024/equipment.

### 2026-05-02 - D&D SRD tools and adventuring gear compendium slice

- Implementation:
  - Added table-driven SRD tool entries for the remaining Artisan's Tools, Other Tools, gaming set variants, and musical instrument variants, while preserving existing Calligrapher's Supplies, Thieves' Tools, and Musical Instrument entries.
  - Expanded D&D SRD tool quick-roll definitions so explicit proficiencies such as Herbalism Kit, Flute, Navigator's Tools, Poisoner's Kit, and individual artisan tools produce first-class `tool-*` sheet actions with the correct ability and proficiency/expertise bonus.
  - Added SRD adventuring gear entries for ammunition variants, arcane focus variants, pack entries, containers, lighting gear, kits, spellcasting focus variants, consumables, Potion of Healing, cantrip/level-1 Spell Scrolls, and the remaining listed mundane gear.
  - Reused the existing equipment purchase path so tools, packs, ammunition, focus variants, kits, and gear can be purchased into actor inventory with cost, weight, quantity, and structured metadata.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test` passed with `25` tests.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test` passed with `2` files and `68` tests.
  ...
  - System SDK tests verify all `130` new tool/adventuring-gear ids resolve, representative tool/ammunition/pack/kit/potion/scroll metadata, expanded tool quick rolls for Herbalism Kit, Flute, and Navigator's Tools expertise, and purchases for Healer's Kit and Arrows.
  - API tests verify representative new tool and gear entries appear in the D&D SRD compendium, and REST purchasing creates Healer's Kit and Arrows inventory with expected cost, quantity, and metadata.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and a real GM bearer token from `POST /api/v1/auth/login`; no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/compendium` returned `200`, `228` entries, and included all `130` newly checked tool and adventuring-gear ids from the slice.
  - A GM-created `Manual SRD Tool Specialist` sheet returned `tool-herbalism-kit: 1d20+2`, `tool-flute: 1d20+2`, and expertise-backed `tool-navigators-tools: 1d20+7`.
  - A fresh Fighter purchased Healer's Kit for `5` GP, leaving `45` GP, with item data `uses: 10`, `action: "utilize"`, and `stabilizesAtZeroHp: true`.
  - The same Fighter purchased two Arrows entries for `2` GP, leaving `43` GP, with item data `ammunition: "arrow"`, `amountPerPurchase: 20`, and `storage: "quiver"`.
  - Source rule references for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf, and D&D Beyond 2024 Basic Rules equipment page, https://www.dndbeyond.com/sources/dnd/br-2024/equipment.

### 2026-05-02 - D&D SRD magic item compendium and protection automation slice

- Implementation:
  - Added a D&D SRD magic-item compendium slice with `48` entries spanning representative armor, ammunition, weapons, shields, rings, wands, staffs, and wondrous items.
  - Added rarity, attunement, crafting-time/cost, charge, resistance-choice, spell-reference, storage-capacity, damage-bonus, save-bonus, and Armor Class bonus metadata for those entries using original short summaries and structured SRD-derived fields.
  - Wired equipped magic-item bonuses into D&D SRD sheet automation: Cloak/Ring of Protection now add to AC and saving throws, Bracers of Defense add AC only when no armor or shield is equipped, Staff of Power adds AC/save/weapon bonuses, and Staff of Striking adds weapon damage bonuses.
  - Extended generic item damage quick rolls and Monk weapon damage overrides so magic `damageBonus` metadata is reflected in normal and versatile item formulas.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test` passed with `25` tests.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test` passed with `2` files and `68` tests after rebuilding `@open-tabletop/system-sdk`.
  - `pnpm --filter @open-tabletop/api build` passed before the manual in-memory API smoke.
  - System SDK tests verify all `48` new magic-item ids resolve, representative magic item metadata for Ammunition of Slaying, Weapon +2, Shield +3, Cloak of Protection, Bracers of Defense, Bag of Holding, and Wand of Magic Missiles, AC/save bonuses from protection items, Bracers of Defense armor/shield exclusion, and Staff of Striking normal/versatile damage formulas.
  - API tests verify representative magic items appear in the D&D SRD compendium and that REST compendium add for Cloak of Protection and Staff of Striking updates the returned sheet with AC, saving throw, charge, and item damage evidence.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and a real GM bearer token from `POST /api/v1/auth/login`; no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/compendium` returned `200`, `276` entries, and included all `48` newly checked magic item ids.
  - Creating `Manual Magic Item Smoke Wizard` from the Wizard template with Criminal/Orc origins returned `200` and actor `act_monyr10hyz1ucvpe`.
  - Adding `cloak-of-protection` returned item `Cloak of Protection`, sheet AC `14`, `armorClassBonus: 1`, and Dexterity save quick roll `1d20+4` with metadata `{ itemBonus: 1 }`.
  - Adding `bracers-of-defense` after the cloak returned sheet AC `16`; adding `chain-mail` afterward returned sheet AC `17`, with only the cloak's `armorClassBonus: 1` still active because Bracers of Defense are ignored while armor is equipped.
  - Adding `staff-of-striking` returned item `Staff of Striking`, `charges.max: 10`, damage quick roll `1d6+2`, and versatile damage quick roll `1d8+2` for the Orc Wizard's Strength modifier plus the staff's magic damage bonus.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD potion and Spell Scroll compendium slice

- Implementation:
  - Added `34` SRD-sourced magic consumable entries: `26` potion-family entries from Philter of Love through Potion of Water Breathing, plus Spell Scroll levels 2-9.
  - Modeled potion rarity, consumable flags, action metadata, healing/damage formulas, save DCs, resistance choices, giant-strength score replacement, condition/effect metadata, duration, and crafting cost/time through the existing magic-item compendium path.
  - Modeled Spell Scroll levels 2-9 with SRD rarity, spell save DC, spell attack bonus, scroll level, consumable, and crafting metadata while preserving the existing cantrip and level-1 scroll entries for compatibility.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test` passed with `25` tests.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test` passed with `2` files and `68` tests.
...
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and a real GM bearer token from `POST /api/v1/auth/login`; no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/compendium` returned `200`, `310` entries, and included all `34` new potion and scroll ids.
  - Creating `Manual SRD Consumables Wizard` from the Wizard template with Criminal/Orc origins returned `200` and actor `act_monz6wuo1x502t85`.
  - Adding `potion-of-healing-superior` returned item `Potion of Healing (superior)`, rarity `rare`, crafting cost `1000` GP, and item healing quick roll `8d4+8`.
  - Adding `potion-of-poison` returned item `Potion of Poison`, item damage quick roll `4d6`, and Constitution save DC `13`.
  - Adding `potion-of-giant-strength-storm` returned item `Potion of Giant Strength (storm)`, rarity `legendary`, and Strength replacement `29`.
  - Adding `spell-scroll-level-9` returned item `Spell Scroll, Level 9`, rarity `legendary`, scroll level `9`, spell save DC `19`, and spell attack bonus `11`.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD expanded monster threat catalog slice

- Implementation:
  - Expanded the D&D SRD monster catalog to `15` unique SRD stat blocks exposed as `16` encounter threat ids, including the existing `goblin-minion` alias.
  - Added SRD-sourced Skeleton, Zombie, Wolf, Dire Wolf, Ogre, Owlbear, Troll, Red Dragon Wyrmling, and Young Red Dragon stat blocks with CR, XP, AC, HP, ability scores, saves, senses, languages, gear, traits, and actions.
  - Added monster action save metadata for breath-weapon style actions, including Red Dragon Wyrmling Fire Breath DC `13` and Young Red Dragon Fire Breath DC `17`, both with Dexterity save and half-damage success metadata.
  - Monster actor sheets expose expanded action quick rolls such as Young Red Dragon `monster-rend-attack`, `monster-rend-damage`, and `monster-fire-breath-damage`.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test` passed with `25` tests.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test` passed with `2` files and `68` tests.
  - `pnpm --filter @open-tabletop/api build` passed before the built-API smoke.
  - Final `git diff --check` passed with only expected LF-to-CRLF working-copy warnings, and final `pnpm check` passed across lint, typecheck, tests, and build; lint and typecheck each reported `22` successful tasks, tests reported `22` successful tasks with API `68` passed and system SDK `25` passed, and build reported `14` successful tasks.
  - System SDK tests verify the `9` new threat ids, representative Skeleton/Ogre/Young Red Dragon stat block metadata, Young Red Dragon encounter difficulty, and Young Red Dragon quick-roll formulas `1d20+10`, `2d6+6+1d6`, and `16d6`.
  - API tests verify the new threat ids in `GET /encounter-threats`, representative threat payloads, deadly encounter planning for Young Red Dragon, monster actor creation from `young-red-dragon`, and returned monster quick rolls.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and a real GM bearer token from `POST /api/v1/auth/login`; no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/encounter-threats` returned `200`, `16` threat ids, and included all `9` newly checked threat ids.
  - Young Red Dragon threat metadata returned CR `10`, `5900 XP`, AC `18`, HP `178`, and Fire Breath `{ damageFormula: "16d6", save: { ability: "dexterity", dc: 17, success: "half" } }`.
  - Planning `1x Young Red Dragon` for a level `10` party actor returned `difficulty: "deadly"`, `partyRating: 3100`, `threatBudget: 5900`, and budgets `{ easy: 1600, standard: 2300, hard: 3100 }`.
  - Creating `Manual Young Red Dragon` from `young-red-dragon` returned actor `act_monzqibbe4i3n236` with HP `{ current: 178, max: 178 }`, AC `18`, CR `10`, and XP `5900`.
  - The returned sheet exposed quick rolls `monster-rend-attack: 1d20+10`, `monster-rend-damage: 2d6+6+1d6`, and `monster-fire-breath-damage: 16d6`.
  - Rolling `monster-fire-breath-damage` with `applyEffect: true` against `Manual Dragon Target` returned `200`, formula `16d6`, and effect `{ type: "damage", pool: "hp", amount: 52, before: 120, after: 68, max: 120 }`.
  - Source rule reference for this slice: official SRD 5.2.1 PDF, https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf.

### 2026-05-02 - D&D SRD humanoid and beast monster staples slice
...
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/system-sdk test` passed with `25` tests.
  - `pnpm --filter @open-tabletop/api test` passed with `2` files and `68` tests after rebuilding the system SDK dist used by the API.
  - `pnpm --filter @open-tabletop/api build` passed before the built-API smoke.
  - Final `git diff --check` passed with only expected LF-to-CRLF working-copy warnings, and final `pnpm check` passed across lint, typecheck, tests, and build; lint and typecheck each reported `22` successful tasks, tests reported `22` successful tasks with API `68` passed and system SDK `25` passed, and build reported `14` successful tasks.
  - System SDK tests verify the `12` new threat ids, representative Bandit Captain/Guard Captain/Giant Spider/Giant Ape stat block metadata, Giant Ape encounter difficulty, and Bandit Captain/Giant Spider monster quick-roll formulas.
  - API tests verify the new threat ids in `GET /encounter-threats`, representative threat payloads, deadly encounter planning for Giant Ape, monster actor creation from `bandit-captain` and `giant-spider`, and returned monster quick rolls.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and a real GM bearer token from `POST /api/v1/auth/login`; no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/encounter-threats` returned `200`, `28` threat ids, and included all `12` newly checked threat ids: `bandit-captain`, `guard`, `guard-captain`, `knight`, `spy`, `warrior-veteran`, `black-bear`, `brown-bear`, `giant-rat`, `giant-spider`, `giant-ape`, and `giant-eagle`.
  - Bandit Captain threat metadata returned budget `450`, CR `2`, AC `15`, HP `52`; creating `SRD Bandit Captain Smoke` returned HP `{ current: 52, max: 52 }`, AC `15`, CR `2`, XP `450`, and quick rolls `monster-pistol-attack: 1d20+5` and `monster-pistol-damage: 1d10+3`.
  - Giant Spider threat metadata returned budget `200`, CR `1`, and Web save `{ ability: "dexterity", dc: 13 }`; creating `SRD Giant Spider Smoke` returned HP `{ current: 26, max: 26 }`, AC `14`, CR `1`, XP `200`, and quick rolls `monster-bite-attack: 1d20+5` and `monster-bite-damage: 1d8+3+2d6`.
...
- Implementation:
  - Added the official SRD condition catalog to the D&D SRD compendium: Blinded, Charmed, Deafened, Exhaustion, Frightened, Grappled, Incapacitated, Invisible, Paralyzed, Petrified, Poisoned, Prone, Restrained, Stunned, and Unconscious.
  - Preserved the existing non-condition tracking entries used by the runtime, including Blessed, Alert, Savage Attacker, and Magic Initiate, while giving Poisoned and Restrained D&D-specific SRD metadata instead of generic-fantasy metadata.
  - Modeled compact structured metadata for VTT behavior such as sight/hearing check failures, speed set to `0`, attack advantage/disadvantage, Strength/Dexterity save failures, close critical hits, Exhaustion level penalties, Prone stand cost, Petrified resistance, and Unconscious dropped-item/unaware state.
  - Extended D&D character import, actor sheet, and REST condition endpoint coverage so SRD conditions can be listed, applied, removed, and imported through the same system runtime path as other compendium-backed conditions.
- Automated validation:
  - `pnpm --filter @open-tabletop/system-sdk test` passed with `25` tests.
  - `pnpm --filter @open-tabletop/system-sdk build` passed.
  - `pnpm --filter @open-tabletop/api test` passed with `2` files and `68` tests after rebuilding the system SDK dist used by the API.
  - `pnpm --filter @open-tabletop/api build` passed before the built-API smoke.
  - Final `git diff --check` passed with only expected LF-to-CRLF working-copy warnings, and final `pnpm check` passed across lint, typecheck, tests, and build; lint and typecheck each reported `22` successful tasks, tests reported `22` successful tasks with API `68` passed and system SDK `25` passed, and build reported `14` successful tasks.
  - System SDK tests verify all `15` official SRD condition ids are present, representative Blinded/Exhaustion/Paralyzed/Poisoned/Restrained/Unconscious metadata, duplicate condition application prevention, D&D sheet rendering for Paralyzed, and D&D import of Paralyzed and Unconscious while skipping unknown conditions.
  - API tests verify `GET /systems/dnd-5e-srd/compendium` exposes all official SRD condition ids and representative Paralyzed/Unconscious metadata, and verify `POST`/`DELETE /conditions` apply and remove Paralyzed and Unconscious on a D&D SRD actor.
- Manual API evidence:
  - Built-API smoke used `apps/api/dist` with `MemoryStateStore` and a real GM bearer token from `POST /api/v1/auth/login`; no legacy `x-user-id` fallback was used.
  - `GET /systems/dnd-5e-srd/compendium` returned `200`, `323` compendium entries, `19` condition-type entries, and included all `15` official SRD condition ids: `blinded`, `charmed`, `deafened`, `exhaustion`, `frightened`, `grappled`, `incapacitated`, `invisible`, `paralyzed`, `petrified`, `poisoned`, `prone`, `restrained`, `stunned`, and `unconscious`.
  - Paralyzed metadata returned `includes: ["incapacitated"]`, `speedSetTo: 0`, failed Strength/Dexterity saving throws, attacks against with advantage, close critical hits, and source `SRD 5.2.1`.
  - Unconscious metadata returned `includes: ["incapacitated", "prone"]`, `dropsHeldItems: true`, `speedSetTo: 0`, failed Strength/Dexterity saving throws, close critical hits, `unaware: true`, and source `SRD 5.2.1`.
  - Creating `SRD Condition Smoke Wizard` returned actor `act_moo0ppg7o9qvr0nt`; applying Paralyzed returned sheet conditions `["paralyzed"]`, applying Unconscious returned `["paralyzed", "unconscious"]`, and deleting Paralyzed returned `["unconscious"]`.
