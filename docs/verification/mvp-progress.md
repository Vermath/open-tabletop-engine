...
- D&D 5.5e SRD is now the primary rules focus with SRD 5.2.1 system registration, Fighter/Cleric/Wizard templates, SRD-oriented compendium entries, spell-slot-consuming action use, target damage/healing application, supported Healing Word/Cure Wounds upcasting, saving throw quick rolls, class save proficiencies, skill and tool check quick rolls, class/template skill and tool proficiencies, SRD currency and equipment purchasing, guided level advancement, character import normalization, SRD monster stat blocks, monster actor creation, monster attack/damage quick rolls, CR/XP threat budgets, 2024 Basic Rules encounter XP thresholds, hit dice, spell slots, short/long rest recovery, API tests, and manual API acceptance evidence. Remaining D&D rules ecosystem work is full SRD-scale content coverage, richer character builder choices, class/subclass feature automation, and full SRD-scale monster/action coverage beyond the current SRD threat catalog. Generic Fantasy, Stellar Frontiers, and Mystic Noir remain available as demo systems for later expansion, but they are no longer the primary gap-closing direction.
