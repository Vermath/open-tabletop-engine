...
- Generic Fantasy now has compendium-backed items, spells, conditions, actor inventory/spell/resource sheet surfaces, condition-aware rolls, item/spell action formulas, spell-slot-consuming action use, character templates, guided level advancement, character import normalization, encounter threat budgets, persisted planned encounters, hit dice, spell slots, resource pools, short/long rest recovery, API tests, and browser/API acceptance evidence. Stellar Frontiers adds a second verified rules system with gear, talents, resource pools, strain-aware sheets, aptitude rolls, gear/talent action formulas, strain/consumable action use, system activation, conditions, character templates, guided rank advancement, character import normalization, encounter threat budgets, short/long rest recovery, API tests, and browser/API acceptance evidence. Mystic Noir adds a third verified rules system with clues, rituals, resource pools, composure-aware sheets, skill rolls, clue/ritual action formulas, resource-consuming action use, campaign activation, conditions, templates, guided case-breakthrough advancement, character import normalization, encounter threat budgets, short/long rest recovery, API tests, browser build coverage, and manual API acceptance evidence. Remaining rules ecosystem work is full SRD-scale content, richer build choices, and deeper automation/content breadth inside each supported system.
- AI flows now cover provider-configured threads, richer permission-filtered prompt context with visible actor action ids, permission-filtered tool advertisement, typed OpenAI Responses tool schemas, Codex loopback proposal-tool and actor-action execution, encounter/journal/scene/token/actor/memory/dice/compendium/action tools, provider retry/failure handling, provider request timeouts, cross-role tool-advertisement permission regression, thread status history, failed-tool observability, invalid tool-input rejection before side effects, provider-backed memory extraction, usage and estimated-cost metrics, GM-only front-end operator telemetry, server-admin cross-campaign AI/Codex operations telemetry, approval/application, generic proposal underlying-permission checks, and deterministic recap memory. Remaining Codex integration work is broader production provider edge cases and future-tool hardening as new tool classes are added. Model-output quality evaluation is intentionally out of scope for the Codex integration goal.
