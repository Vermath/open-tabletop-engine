...
- D&D 5.5e SRD is now the primary rules focus with SRD 5.2.1 system registration, Fighter/Cleric/Wizard templates, SRD character origin catalogs, create-time background/species application, SRD-oriented compendium entries, bounded expanded spell/weapon/armor compendium coverage, computed Armor Class from SRD armor/shield items, fixed and secondary spell damage formulas, spell-slot-consuming action use, target damage/healing application, supported Healing Word/Cure Wounds/Chromatic Orb/Ray of Sickness/Ice Knife upcasting, saving throw quick rolls, class save proficiencies, skill and tool check quick rolls, class/template/background skill and tool proficiencies, SRD currency and equipment purchasing, guided level advancement, character import normalization, SRD monster stat blocks, monster actor creation, monster attack/damage quick rolls, CR/XP threat budgets, 2024 Basic Rules encounter XP thresholds, hit dice, spell slots, short/long rest recovery, API tests, and manual API acceptance evidence. Remaining D&D rules ecosystem work is full SRD-scale spell/equipment/magic-item content coverage, deeper class/subclass feature automation, more complete species trait automation, and full SRD-scale monster/action coverage beyond the current SRD threat catalog. Generic Fantasy, Stellar Frontiers, and Mystic Noir remain available as demo systems for later expansion, but they are no longer the primary gap-closing direction.
