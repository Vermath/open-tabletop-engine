...
| FF-027 | The atomic scene-duplication E2E could spend its full timeout waiting for post-login UI after a transient first-click **Failed to fetch**, without ever exercising duplication. | Isolated the feature assertions from the login precondition; after FF-032 moved recovery into the seeded-demo product path, the one-off test retry was removed while the forced commit failure and zero-partial-copy assertions remained unchanged. | Fresh-server focused journey passed 5/5; the first-login abort is now covered centrally by FF-032. |
...
| FF-034 | Under full-suite CPU and I/O contention, the gridless-scene and reviewed typed-damage integration tests could exceed Vitest's generic 5-second default even though immediate focused reruns completed in 1.970 and 2.446 seconds. | Assigned each multi-request API integration test the existing 15-second per-test allowance used by comparable API journeys. Those explicit guards remain; FF-035 subsequently corrected the same budget at the functional-project boundary. | Both focused files passed five sequential reruns and a contention run; API typecheck and the focused Vitest suite passed. No additional Browser capture was required. |
| FF-035 | Successive clean exact-commit cold suites hit the same generic 5-second timeout in three unrelated full-app API journeys: gridless scenes, reviewed typed damage, and actual request logging. The functional suite contains 588 `buildApp` invocations across 95 test files, so isolated exemptions did not match the category's contention profile. | Set a 15-second timeout only on the `api-functional` Vitest project and added a config-contract regression. Retries remain disabled; the isolated performance project keeps its default timeout and explicit budgets; the intentional timeout diagnostic remains excluded at 3 seconds; fixture cleanup remains 2.5 seconds; production behavior is unchanged. | The six-file affected cohort passed 14 tests in each of five sequential runs (70 executions total, 22.48-24.22 seconds per run). No additional Browser capture was required. |
...
| Gate | Result |
|---|---|
| Focused regressions for FF-001 through FF-035 | Passed during remediation |
