...
System runtime endpoints are campaign-scoped. The runtime catalog currently includes `generic-fantasy` and `stellar-frontiers`. Generic Fantasy exposes items, spells, and conditions with ability quick rolls; Stellar Frontiers exposes sci-fi gear, talents, strain-aware actor sheets, aptitude quick rolls, and conditions. Both systems can add compendium entries to actors, apply and remove actor conditions, summarize actor sheets with system-specific inventory/spell/talent surfaces, and produce condition-aware quick-roll dice formulas that can be posted into chat.
