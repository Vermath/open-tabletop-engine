...
The D&D SRD magic item slice now covers `48` representative SRD entries across armor, ammunition, weapons, shields, rings, wands, staffs, and wondrous items. `GET /api/v1/campaigns/{campaignId}/systems/dnd-5e-srd/compendium` returns rarity, attunement, crafting, charge, resistance-choice, spell-reference, storage-capacity, bonus, and item-category metadata for those entries, and `POST /api/v1/campaigns/{campaignId}/systems/dnd-5e-srd/actors/{actorId}/compendium` attaches them through the same permissioned actor inventory path as mundane gear. Actor sheets now apply supported magic item automation for equipped AC bonuses, saving throw bonuses, Bracers of Defense no-armor/no-shield exclusion, and Staff of Striking item damage formulas.
