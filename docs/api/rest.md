...
The D&D SRD monster threat slice now exposes `16` threat ids backed by `15` unique SRD stat blocks across humanoids, undead, beasts, giants, a monstrosity, and red dragons from CR `1/8` through CR `10`. `GET /api/v1/campaigns/{campaignId}/systems/dnd-5e-srd/encounter-threats` returns expanded action, trait, save, CR, XP, HP, and AC metadata for threats such as Skeleton, Ogre, Troll, Red Dragon Wyrmling, and Young Red Dragon. `POST /api/v1/campaigns/{campaignId}/systems/dnd-5e-srd/monsters` can create those monster actors, and Young Red Dragon sheets expose `monster-rend-attack`, `monster-rend-damage`, and `monster-fire-breath-damage` quick rolls, with Fire Breath carrying Dexterity save DC `17` and half-damage success metadata.
