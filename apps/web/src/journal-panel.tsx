...
import { apiDelete, apiGet, apiPatch, type CampaignSessionInfo, type Snapshot } from "./api.js";
...
interface JournalCreateDraft extends Omit<JournalDraft, "expectedUpdatedAt"> {}
...
export function storedJournalDraft(value: unknown): JournalDraft | undefined {
  if (!value || typeof value !== "object") return undefined;
  const draft = value as Partial<JournalDraft>;
  if ((draft.kind !== "entry" && draft.kind !== "folder") || typeof draft.title !== "string" || typeof draft.body !== "string" || typeof draft.tags !== "string") return undefined;
  ...
  if (!Array.isArray(draft.links) || !draft.links.every((link) => link && typeof link === "object" && typeof (link as JournalEntityLink).id === "string" && typeof (link as JournalEntityLink).targetId === "string" && typeof (link as JournalEntityLink).targetType === "string")) return undefined;
...
interface JournalPanelProps {
  ...
  onGenerateRecap(visibility: Visibility, idempotencyKey: string, sessionId?: string): Promise<void>;
...
export function JournalPanel(props: JournalPanelProps) {
  ...
  const recapSessions = useMemo(() => [...(props.sessions ?? [])]
    .filter((session) => session.status !== "planned" && Boolean(session.startedAt))
    .sort((left, right) => (right.endedAt ?? right.startedAt ?? right.createdAt).localeCompare(left.endedAt ?? left.startedAt ?? left.createdAt)), [props.sessions]);
  const createDraftKey = localDraftKey("journal-create", props.campaignId, props.currentUserId);
  const [recoveredCreateDraft] = useState(() => storedJournalDraft(readLocalDraft<unknown>(createDraftKey)) as JournalCreateDraft | undefined);
  const [title, setTitle] = useState(recoveredCreateDraft?.title ?? "");
  const [body, setBody] = useState(recoveredCreateDraft?.body ?? "");
  const [visibility, setVisibility] = useState<Visibility>(recoveredCreateDraft?.visibility ?? "gm_only");
  const [tags, setTags] = useState(recoveredCreateDraft?.tags ?? "prep");
  const [visibleToUserIds, setVisibleToUserIds] = useState<string[]>(recoveredCreateDraft?.visibleToUserIds ?? []);
  const [visibleToActorIds, setVisibleToActorIds] = useState<string[]>(recoveredCreateDraft?.visibleToActorIds ?? []);
  const [kind, setKind] = useState<"folder" | "entry">(recoveredCreateDraft?.kind ?? "entry");
  const [parentId, setParentId] = useState(recoveredCreateDraft?.parentId ?? "");
  const [links, setLinks] = useState<JournalEntityLink[]>(recoveredCreateDraft?.links ?? []);
  const [draftTouched, setDraftTouched] = useState(Boolean(recoveredCreateDraft));
  const [createDrawerOpen, setCreateDrawerOpen] = useState(Boolean(recoveredCreateDraft));
  const [draftPersistence, setDraftPersistence] = useState<DraftPersistenceStatus>(recoveredCreateDraft ? "saved" : "idle");
  const [creating, setCreating] = useState(false);
  const [createError, setCreateError] = useState("");
  const [createStatus, setCreateStatus] = useState("");
  const [recapBusy, setRecapBusy] = useState(false);
  const [recapError, setRecapError] = useState("");
  const [recapSessionId, setRecapSessionId] = useState(() => recapSessions[0]?.id ?? "");
  const createAttemptRef = useRef<{ fingerprint: string; key: string } | null>(null);
  const recapAttemptRef = useRef<string | null>(null);
  const targetsValid = journalVisibilityHasTargets(visibility, visibleToUserIds, visibleToActorIds);
  const recapSession = recapSessions.find((session) => session.id === recapSessionId);

  useEffect(() => {
    if (recapSessionId && !recapSessions.some((session) => session.id === recapSessionId)) setRecapSessionId(recapSessions[0]?.id ?? "");
  }, [recapSessionId, recapSessions]);
  ...
  useEffect(() => {
    ...
    setDraftPersistence(draftPersistenceStatus(writeLocalDraft(createDraftKey, {
      kind,
      ...
      tags,
      links
    } satisfies JournalCreateDraft)));
  }, [body, createDraftKey, draftTouched, kind, links, parentId, tags, title, visibility, visibleToActorIds, visibleToUserIds]);

  async function createEntry() {
    if (creating || !targetsValid) return;
    const fingerprint = JSON.stringify({ kind, parentId, title: title.trim(), body: body.trim(), visibility, visibleToUserIds, visibleToActorIds, tags, links });
    ...
    try {
      await props.onCreate({
        ...
        body,
        visibility,
        visibleToUserIds: visibility === "specific_players" ? visibleToUserIds : [],
        visibleToActorIds: visibility === "specific_characters" ? visibleToActorIds : [],
        tags,
        links,
        idempotencyKey: createAttemptRef.current.key,
      });
      removeLocalDraft(createDraftKey);
  ...
  function discardCreateDraft() {
    ...
    setDraftTouched(false);
    setDraftPersistence("idle");
    setCreateStatus("Draft discarded.");
  }
  ...
  async function generateRecap() {
    ...
    setRecapError("");
    try {
      await props.onGenerateRecap(visibility, recapAttemptRef.current, recapSessionId || undefined);
      recapAttemptRef.current = null;
    } catch (failure) {
    ...
    <div className="panel-stack">
      ...
      {props.canCreate && (
        <div className="button-row wrap">
          <label><span>Recap scope</span><select aria-label="Recap scope" value={recapSessionId} disabled={recapBusy} onChange={(event) => setRecapSessionId(event.target.value)}><option value="">Campaign activity</option>{recapSessions.map((session) => <option key={session.id} value={session.id}>Session {session.number}: {session.title}</option>)}</select></label>
          <button className="ghost-button" type="button" disabled={recapBusy} onClick={() => void generateRecap()}>
            <ScrollText size={15} /> {recapBusy ? "Generating recap..." : recapSession ? `Generate Session ${recapSession.number} recap` : "Generate campaign activity recap"}
          </button>
        </div>
      )}
      ...
      {props.canCreate && (
        <details
          ...
          onToggle={(event) => setCreateDrawerOpen(event.currentTarget.open)}
        >
          ...
          <form className="operator-section content-import-form create-drawer-form" onSubmit={(event) => { event.preventDefault(); void createEntry(); }}>
            <DraftPersistenceNotice subject="Journal" status={draftPersistence} />
            <label><span>Type</span><select aria-label="Journal record type" value={kind} disabled={creating} onChange={(event) => { setDraftTouched(true); setKind(event.target.value as "folder" | "entry"); }}><option value="entry">Entry</option><option value="folder">Folder</option></select></label>
            <label><span>Parent folder</span><select aria-label="Journal parent folder" value={parentId} disabled={creating} onChange={(event) => { setDraftTouched(true); setParentId(event.target.value); }}><option value="">Top level</option>{folders.map((folder) => <option key={folder.id} value={folder.id}>{folder.title}</option>)}</select></label>
            <label><span>Title</span><input aria-label="Journal title" value={title} disabled={creating} placeholder={kind === "folder" ? "Lore" : "Session note"} onChange={(event) => { setDraftTouched(true); setTitle(event.target.value); }} /></label>
            <label><span>Visibility</span><select aria-label="Journal visibility" value={visibility} disabled={creating} onChange={(event) => { setDraftTouched(true); setVisibility(event.target.value as Visibility); }}><option value="gm_only">GM only</option><option value="public">Public</option><option value="specific_players">Specific players</option><option value="specific_characters">Specific characters</option></select></label>
            <JournalAudienceFields visibility={visibility} members={props.members} actors={props.actors} visibleToUserIds={visibleToUserIds} visibleToActorIds={visibleToActorIds} setVisibleToUserIds={(value) => { setDraftTouched(true); setVisibleToUserIds(value); }} setVisibleToActorIds={(value) => { setDraftTouched(true); setVisibleToActorIds(value); }} disabled={creating} />
            <label><span>Tags</span><input aria-label="Journal tags" value={tags} disabled={creating} placeholder="prep, clue" onChange={(event) => { setDraftTouched(true); setTags(event.target.value); }} /></label>
            <label><span>Body</span><textarea aria-label="Journal body" value={body} disabled={creating} onChange={(event) => { setDraftTouched(true); setBody(event.target.value); }} /></label>
            <JournalLinksEditor links={links} setLinks={(value) => { setDraftTouched(true); setLinks(value); }} targets={props.linkTargets} disabled={creating} />
            ...
            <button className="primary-button wide" type="submit" disabled={!title.trim() || !targetsValid || creating}><Plus size={16} /> {creating ? "Creating..." : `Create ${kind}`}</button>
            {draftTouched && <button className="ghost-button wide" type="button" disabled={creating} onClick={discardCreateDraft}><Trash2 size={14} /> Discard journal draft</button>}
          </form>
        </details>
      )}
...
function JournalLinksEditor({ links, setLinks, targets, disabled = false }: JournalLinksEditorProps) {
  ...
  return (
    <fieldset className="operator-section journal-links-editor" disabled={disabled}>
      ...
      <button className="ghost-button" type="button" disabled={disabled || !targetValue} onClick={addLink}><Link2 size={14} /> Add link</button>
    </fieldset>
  );
}
...
function JournalEntryCard(props: JournalEntryCardProps) {
  ...
  async function save(input: JournalDraft): Promise<boolean> {
    return run(() => props.onUpdate(input), "Journal changes saved and queued for canon review.");
  }
  ...
  return (
    <article id={campaignSearchAnchorId("journal", props.journal.id)} className="journal-entry" data-depth={Math.min(props.depth, 6)} aria-label={`Journal ${kind} ${props.journal.title}`} tabIndex={-1}>
      ...
      {editing && <JournalEntryEditor key={revisionStateKey} campaignId={props.campaignId} currentUserId={props.currentUserId} journal={props.journal} folders={props.folders} members={props.members} actors={props.actors} linkTargets={availableLinkTargets} busy={busy} onSave={save} onSaved={() => setEditing(false)} onCancel={() => setEditing(false)} />}
    </article>
  );
}
...
interface JournalEntryEditorProps {
  ...
  linkTargets: JournalLinkTargetOption[];
  busy: boolean;
  onSave(input: JournalDraft): Promise<boolean>;
  onSaved(): void;
  onCancel(): void;
}

/** Keep the recoverable browser draft unless the server confirmed the update. */
export async function clearJournalDraftAfterConfirmedSave<T>(
  draft: T,
...
export function JournalEntryEditor({ campaignId, currentUserId, journal, folders, members, actors, linkTargets, busy, onSave, onSaved, onCancel }: JournalEntryEditorProps) {
  const draftStorageKey = localDraftKey("journal-edit", campaignId, currentUserId, journal.id);
  ...
  const recoveredDraft = storedJournalDraft(readLocalDraft<unknown>(draftStorageKey));
  const [draft, setDraft] = useState<JournalDraft>(() => recoveredDraft ? { ...recoveredDraft, expectedUpdatedAt: journal.updatedAt } : initialDraft);
  ...
  useEffect(() => {
    if (draftTouched) setDraftPersistence(draftPersistenceStatus(writeLocalDraft(draftStorageKey, draft)));
  }, [draft, draftStorageKey, draftTouched]);
  const targetsValid = journalVisibilityHasTargets(draft.visibility, draft.visibleToUserIds, draft.visibleToActorIds);
  const updateDraft = (update: (current: JournalDraft) => JournalDraft) => {
    if (busy) return;
  ...
  const saveDraft = async () => {
    if (busy) return;
    const saved = await clearJournalDraftAfterConfirmedSave(draft, onSave, () => removeLocalDraft(draftStorageKey));
    if (!saved) return;
    setDraftTouched(false);
    setDraftPersistence("idle");
    onSaved();
  ...
  return (
    <form className="operator-section content-import-form journal-edit-form" aria-label={`Edit journal ${journal.title}`} onSubmit={(event) => { event.preventDefault(); void saveDraft(); }}>
      <DraftPersistenceNotice subject="Journal edit" status={draftPersistence} />
      <label><span>Type</span><select aria-label="Edit journal record type" value={draft.kind} disabled={busy} onChange={(event) => updateDraft((current) => ({ ...current, kind: event.target.value as "folder" | "entry" }))}><option value="entry">Entry</option><option value="folder">Folder</option></select></label>
      <label><span>Parent folder</span><select aria-label="Edit journal parent folder" value={draft.parentId ?? ""} disabled={busy} onChange={(event) => updateDraft((current) => ({ ...current, parentId: event.target.value || undefined }))}><option value="">Top level</option>{folders.filter((folder) => folder.id !== journal.id).map((folder) => <option key={folder.id} value={folder.id}>{folder.title}</option>)}</select></label>
      <label><span>Title</span><input aria-label="Edit journal title" value={draft.title} required disabled={busy} onChange={(event) => updateDraft((current) => ({ ...current, title: event.target.value }))} /></label>
      <label><span>Visibility</span><select aria-label="Edit journal visibility" value={draft.visibility} disabled={busy} onChange={(event) => updateDraft((current) => ({ ...current, visibility: event.target.value as Visibility }))}><option value="gm_only">GM only</option><option value="public">Public</option><option value="specific_players">Specific players</option><option value="specific_characters">Specific characters</option></select></label>
      <JournalAudienceFields visibility={draft.visibility} members={members} actors={actors} visibleToUserIds={draft.visibleToUserIds} visibleToActorIds={draft.visibleToActorIds} setVisibleToUserIds={(value) => updateDraft((current) => ({ ...current, visibleToUserIds: value }))} setVisibleToActorIds={(value) => updateDraft((current) => ({ ...current, visibleToActorIds: value }))} disabled={busy} />
      <label><span>Tags</span><input aria-label="Edit journal tags" value={draft.tags} disabled={busy} onChange={(event) => updateDraft((current) => ({ ...current, tags: event.target.value }))} /></label>
      <label><span>Body</span><textarea aria-label="Edit journal body" value={draft.body} rows={5} disabled={busy} onChange={(event) => updateDraft((current) => ({ ...current, body: event.target.value }))} /></label>
      <JournalLinksEditor links={draft.links} setLinks={(links) => updateDraft((current) => ({ ...current, links }))} targets={linkTargets} disabled={busy} />
      <div className="button-row wrap"><button className="primary-button" type="submit" disabled={busy || !draft.title.trim() || !targetsValid}><Save size={14} /> Save changes</button><button className="ghost-button" type="button" disabled={busy} onClick={onCancel}><X size={14} /> Close and keep draft</button>{draftTouched && <button className="ghost-button" type="button" disabled={busy} onClick={discardDraft}><Trash2 size={14} /> Discard draft</button>}</div>
    </form>
  );
}
