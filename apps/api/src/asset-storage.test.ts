...
import { assetStorageKey, LocalAssetStorage, S3AssetStorage } from "./asset-storage.js";

describe("S3AssetStorage", () => {
  it("retries bucket readiness after a transient failure", async () => {
    const storage = new S3AssetStorage({ bucket: "configured-assets", region: "us-east-1", forcePathStyle: false });
    let headAttempts = 0;
    const send = vi.fn(async (command: unknown) => {
      if (command instanceof HeadBucketCommand && ++headAttempts === 1) throw new Error("temporary MinIO startup failure");
      return {};
    });
    ...
    await expect(storage.put(asset, Buffer.from([1, 2, 3]))).rejects.toThrow("temporary MinIO startup failure");
    await expect(storage.put(asset, Buffer.from([1, 2, 3]))).resolves.toMatchObject({ provider: "s3", bucket: "configured-assets" });
  ...
  it("fails closed instead of reinterpreting a foreign-bucket key in the configured bucket", async () => {
    ...
    for (const bucket of [undefined, ""]) {
      const missingBucketAsset = { ...asset, storage: { provider: "s3" as const, bucket, key: "foreign/secret.png" } };
...
describe("asset storage keys", () => {
  it("keeps archive-controlled identifiers collision-resistant", async () => {
    ...
    try {
      ...
      await expect(storage.read(unsafe)).resolves.toEqual(Buffer.from("unsafe"));
      await expect(storage.read(canonical)).resolves.toEqual(Buffer.from("canonical"));
    } finally {
